...
namespace CardDetectionExample
{
    class CardDetector
    {
        ...
        public void DetectCard_SobelXY(string imagePath)
        {
            // Load the image using OpenCVSharp4
            Mat image = Cv2.ImRead(imagePath);
            ...
            using (Mat gray = new Mat())
            {
                ...
                using (Mat blurred = new Mat())
                {
                    ...
                    using (Mat edgesX = new Mat())
                    using (Mat edgesY = new Mat())
                    {
                        Cv2.Sobel(blurred, edgesX, MatType.CV_8U, 1, 0, ksize: 3, scale: 1, delta: 0, BorderTypes.Default);
                        Cv2.Sobel(blurred, edgesY, MatType.CV_8U, 0, 1, ksize: 3, scale: 1, delta: 0, BorderTypes.Default);

                        // Combine the X and Y edge images using bitwise OR
                        using (Mat edges = new Mat())
                        {
                            Cv2.BitwiseOr(edgesX, edgesY, edges);
